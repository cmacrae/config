...
#+startup: overview
* TODO About
Introduce how we compose our configuration & update the description below
...
#+begin_src emacs-lisp :tangle early-init.el :early:
...
Packages are managed via Nix, so disable =package.el= support.
#+begin_src emacs-lisp :tangle early-init.el :early:
