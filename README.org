...
*** net1
A Raspberry Pi 4 that acts as a DHCP (Kea) server, DNS (unbound) server, proxy (nginx) server for upstream media services, and Wireguard gateway for my home network
